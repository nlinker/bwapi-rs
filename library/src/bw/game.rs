...
impl Game {
    ...
    pub fn issue_command(&mut self, units: &Unitset, command: UnitCommand) -> bool {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
    ...
    pub fn observers(&mut self) -> Playerset {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        Playerset {
    ...
    pub fn ping_minimap(&mut self, p: Position) {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        g.pingMinimap(p)
    }
    ...
    pub fn send_text_ex(&mut self, to_allies: bool, text: &str) {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        ffi::_game_sendTextEx(g, to_allies, text)
    }
    pub fn set_alliance(&mut self, player: &Player, allied: bool, allied_victory: bool) -> bool {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        unsafe { g.setAlliance(player.raw.as_ptr(), allied, allied_victory) }
    }
    pub fn set_command_optimization_level(&mut self, level: i32) {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
    ...
    pub fn set_lat_com(&mut self, is_enabled: bool) {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
    ...
    pub fn set_map(&mut self, text: &str) -> bool {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        ffi::_game_setMap(g, text)
    }
    pub fn set_reveal_all(&mut self, reveal: bool) -> bool {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        g.setRevealAll(reveal)
    }
    pub fn set_screen_position(&mut self, p: Position) {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        g.setScreenPosition(p)
    }
    pub fn set_vision(&mut self, player: &Player, enabled: bool) -> bool {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
    ...
    pub fn draw_text(&mut self, ct: CoordinateType, x: i32, y: i32, text: &str) {
        let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(self.raw.as_mut().unwrap().as_mut()) };
        ffi::_game_drawText(g, ct, x, y, text);
    }
    pub fn draw_text_map(&mut self, x: i32, y: i32, text: &str) {
