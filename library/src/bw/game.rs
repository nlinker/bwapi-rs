...
impl Game {
    ...
    // pub fn get_all_units(&self) { let g: &ffi::Game = unsafe { &*self.raw }; g.getAllUnits()  }                                                                                                       //                  (self: &Game) -> &Unitset
    // pub fn get_apm(&self) { let g: &ffi::Game = unsafe { &*self.raw }; g.getAPM()  }                                                                                                       //                        (self: &Game, includeSelects: bool) -> i32
    // pub fn get_average_fps(&self) { let g: &ffi::Game = unsafe { &*self.raw }; g.getAverageFPS()  }                                                                                                       //                (self: &Game) -> f64
    ...
    // pub fn self_(&self) -> Player { let g: &ffi::Game = unsafe { &*self.raw }; ffi::_game_self
    // pub fn send_text(&self) { let g: Pin<&mut ffi::Game> = unsafe { Pin::new_unchecked(&mut *self.raw) }; g._game_sendText()  }                                     //                          (game: Pin<&mut Game>, text: &str)
